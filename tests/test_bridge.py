...
class TestBridgeIPC(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_bridge_edge_cases(self):
        ...
        d2 = CommData("s1", "srv1", "ch", "ev", {}, path=["server-b"])
    ...
    async def test_bridge_rpc_and_broadcast(self):
        ...
        await ch_a.add_event("greet", call=lambda cd: f"Hello {cd.data['name']}", parameters={"name": str})
        ...
        ch_b.on_receive(lambda cd: received.set(), event_name="shout")
